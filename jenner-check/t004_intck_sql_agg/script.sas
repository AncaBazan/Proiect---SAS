/* 5. Utilizarea de functii SAS (Program 1.sas, section 5): INTCK() date
   arithmetic plus PROC SQL SUM/GROUP BY aggregation. Original reads
   ...
   the repo's own Electronice_vanzari2.xlsx, and today() is pinned to a
   fixed reference date so the elapsed-days output is reproducible. */
data Functii;
...
/* Suma totala a vanzarilor */
proc sql;
 select sum(Vanzari_totale) as Suma_totala_vanzari
 from Functii;
quit;

/* Diferenta in zile intre data lansarii si o data de referinta fixa
   (inlocuieste today() pentru reproductibilitate) */
data dif_data;
 set Functii;
 Zile_diferenta = intck('day', Data_lansare, '01JAN2024'd);
run;
...
/* Numarul de inregistrari pentru fiecare categorie */
proc sql;
